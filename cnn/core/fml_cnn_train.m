function [net, info] = fml_cnn_train(net, imdb, getBatch, varargin)
% fork of CNN_TRAIN, set to use FML variants to support
%   3d convolutions, max-pooling
%   as well as better support for binary output variables
%
...
% fml_simplenn_display(net, 'batchSize', opts.batchSize) ; % FML addition
...
numGpus = numel(opts.gpus) ;
if numGpus >= 1
  net = fml_simplenn_move(net_cpu, 'gpu') ; % FML addition
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    % FML addition
    res = fml_simplenn(net, im, dzdy, res, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
...
if numGpus >= 1
  net_cpu = fml_simplenn_move(net, 'cpu') ; % FML addition
else
  net_cpu = net ;
end
