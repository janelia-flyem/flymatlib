  ...
  switch l.type
    ...
    case 'convt'
      ...
      res(i+1).x = vl_nnconvt(res(i).x, l.weights{1}, l.weights{2}, ...
        ...
        l.opts{:}, ...
        cudnn{:}) ;
      else % FML addition - quick "3d" by collapsing 2nd,3rd dims
        assert(size(l.weights{1},2)==1, 'FML:AssertionFailed', ...
               'currently only works if y filter dim == 1');
        x_sz = size(res(i).x);
        if(length(x_sz)==4), x_sz(5) = 1; end
        x_in  = reshape(res(i).x, x_sz(1), [], x_sz(4), x_sz(5));
        x_out = vl_nnconvt(x_in, l.weights{1}, l.weights{2}, ...
                           'crop', l.crop, ...
                           'upsample', l.upsample, ...
                           'numGroups', l.numGroups, ...
                           l.opts{:}, ...
                           cudnn{:}) ;
        res(i+1).x = reshape(x_out, size(x_out,1), [], ...
                             x_sz(3), size(x_out,3), x_sz(5));
      end
    ...
    switch l.type
      ...
      case 'convt'
        ...
        [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
          ...
          if(length(x_sz)==4), x_sz(5) = 1; end
          x_in    = reshape(res(i).x, x_sz(1), [], x_sz(4), x_sz(5));
          dzdx_in = reshape(res(i+1).dzdx, size(res(i+1).dzdx,1), ...
                            [], size(res(i+1).dzdx,4), x_sz(5));

          [dzdx_out, dzdw{1}, dzdw{2}] = ...
              vl_nnconvt(x_in, l.weights{1}, l.weights{2}, dzdx_in, ...
                         'crop', l.crop, ...
                         'upsample', l.upsample, ...
                         'numGroups', l.numGroups, ...
                         l.opts{:}, ...
