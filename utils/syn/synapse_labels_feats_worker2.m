function [lltt, fftt, syntt, locstt] = synapse_labels_feats_worker2(...
  ...
  for ww = 1:n_windows
    ...
    tmp_dl = double(convolution3D_FFTdomain( ...
        double(local_seg{ww} == tbar_seg), ...
        dilate_flt{1}) > 1e-4);
    tmp_dl = tmp_dl - (local_seg{ww}==tbar_seg);
    tmp_cc = bwconncomp(tmp_dl);
    if(length(tmp_cc.PixelIdxList)>1)
      tmp_sz = cellfun(@length,tmp_cc.PixelIdxList);
      [~,tmp_id] = max(tmp_sz);
      ts_mask{ww,dd} = zeros(size(ts_mask{ww,dd}));
      ts_mask{ww,dd}(tmp_cc.PixelIdxList{tmp_id})=1;
    end
    % fprintf('%d\n', length(tmp_cc.PixelIdxList));

    local_seg{ww}   = local_seg{ww} .* window_flt{ww};

    if(~isnan(window_radii(2,ww)) || ...
...
function feats = get_features(...
  ...
  for wwc = 1:n_windows
    for ddc = 1:n_dilate
      ...
      in_mask = double(ts_mask{wwc,ddc} & (local_seg{wwc}==ss));
